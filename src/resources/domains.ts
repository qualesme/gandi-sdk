...
export class DomainsResource {
	...
	createNewDomain(opts: CreateDomainPayload, dryRun = false, sharingId?: any) {
		...
		if (sharingId) params.sharing_id = sharingId;
		if (dryRun) headers.dry_run = true;
		return this.client.post(`/domain/domains`, opts, params, headers);
	}
	...
	activateDNSSECWithLiveDNS(domain: string, sharingId?: any) {
		...
		if (sharingId) params.sharing_id = sharingId;
		return this.client.post(
			`/domain/domains/${encodeURIComponent(domain)}/livedns/dnssec`,
			undefined,
			params,
		);
	}
	...
	resentReachabilityEmail(domain: string) {
		return this.client.patch(`/domain/domains/${encodeURIComponent(domain)}/reachability`, {
			action: 'resend',
		});
	}
	...
	transferLock(domain: string, lock: boolean) {
		return this.client.patch(`/domain/domains/${encodeURIComponent(domain)}/status`, {
	...
	acceptOrDeclineDomainTransfer(domain: string, accept: boolean, authInfo: string) {
		return this.client.post(`/domain/domains/${encodeURIComponent(domain)}/transferout`, {
			accept,
			authInfo,
		});
	}
	...
	transferDomainToGandi(opts: TransferInDomainPayload, dryRun = false, sharingId?: any) {
		...
		if (sharingId) params.sharing_id = sharingId;
		if (dryRun) headers.dry_run = true;
		return this.client.post(`/domain/transferin`, opts, params, headers);
	}
	...
	checkTransferAvailability(domain: string, authinfo: string) {
		return this.client.post(`/domain/transferin/${encodeURIComponent(domain)}/available`, {
			authinfo,
		});
	}
